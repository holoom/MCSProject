...
legend('North', 'East')
saveas(gcf, fullfile(sub_path  , 'Translational_ekfsim7.png')); 
...
saveas(gcf, fullfile(sub_path  , 'Rotational_ekfsim7.png')); 
...
saveas(gcf, fullfile(sub_path  , 'xy_ekfsim7.png')); 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%% without observer 


sub_path = 'figs/sim7/without_obs' ; 
if ~exist(sub_path, 'dir')
    mkdir(pwd, sub_path);
end 
...
title('Full DP system (Translational motion)'); 
legend('North', 'East')
saveas(gcf, fullfile(sub_path  , 'Translational_noobs_sim7.png')); 
...
ylabel('Amplitude (rad) ')
title('Full DP system (Rotational motion)'); 
saveas(gcf, fullfile(sub_path  , 'Rotational_noobs_sim7.png')); 
...
ylabel('Amplitude in North direction (m)')
legend('actual trajectory', 'desired trajectory')
saveas(gcf, fullfile(sub_path  , 'xy_noobs_sim7.png')); 
...
% sim2 parameters
observer = 3; % withoutobserver = 1, EKF = 2, NonlinearObserver = 3
use_env_forces = 1;      % 0 for no environmental forces , 1 for enalbling the environmental forces
enable_waves = 1;        % 0 for not using waves, 1 for enabling the waves
allocation_method = 1;   % 0 for quadratic programming method , and 1 for pseudo inverse method
ref_model = 0;    % 0 for 0 desired setpoint , 1 for 4 corners     
...
title('Full DP system (Translational motion)'); 
legend('North', 'East')
saveas(gcf, fullfile(sub_path  , 'Translational_npo_sim7.png')); 
...
ylabel('Amplitude (rad) ')
title('Full DP system (Rotational motion)'); 
saveas(gcf, fullfile(sub_path  , 'Rotational_NPO_sim7.png')); 
...
ylabel('Amplitude in North direction (m)')
legend('actual trajectory', 'desired trajectory')
saveas(gcf, fullfile(sub_path  , 'xy_npo_sim7.png')); 
